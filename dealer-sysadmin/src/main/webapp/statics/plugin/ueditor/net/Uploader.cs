    ...
    public Hashtable upFile(HttpContext cxt, string pathbase, string[] filetype, int size)
    {
        ...
        try
        {
            ...
            if (state == "SUCCESS")
            {
                ...
                string sFileName = dt.ToString("yyyyMMddHHmmssffff");  //文件名称

                Stream stream;
                string fileExtension = System.IO.Path.GetExtension(uploadFile.FileName).ToLower();
                string DirectoryPath;
                DirectoryPath = pathbase;

                string retPath = string.Empty;
                if (FTPHelper.checkDirectory(DirectoryPath, dt.Year.ToString()) && FTPHelper.checkDirectory(DirectoryPath + "\\" + dt.Year.ToString(), dt.Month.ToString()))
                {
                    string FullPath = DirectoryPath + "/" + dt.Year.ToString() + "/" + dt.Month.ToString() + "/" + sFileName + fileExtension;//最终文件路径
                    stream = uploadFile.InputStream;
                    FTPHelper.UploadFileByFTP(stream, FullPath);

                    URL = "http://i0.chexun.net/i/" + FullPath;
                }
            }
        }
        catch (Exception e)
