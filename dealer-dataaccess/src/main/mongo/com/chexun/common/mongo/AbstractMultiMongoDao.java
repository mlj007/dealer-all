 ...
 * @param <T>
 */
public abstract class AbstractMultiMongoDao<T> {
...
//			System.out.println("db=" + db);
	 ...
	 * 初始化操作MongoDb中的数据集的配置项（下面的实现主要是得到该操作集得MongoDB对象，并设置该文档所对应的java对象）
	 */
	protected void initialize_read() {
		if (!init_read) {
			ChexunMongoOptions mo = (ChexunMongoOptions) mongo_read
					.getMongoOptions();
			db_read = mongo_read.getDB(mo.getDbname());
...
//				System.out.println("2222222222222=="+cursor.next().get("_id").toString());
	...
	public final void saveOrUpdate4(String collectName,T object) {
		try {
			if(!init){
				initialize();
			}
			DBCollection collect = db.getCollection(collectName);
			DBObject dbObject = covnToDBObject(object);
			T t1 = this.getByStr(collectName,String.valueOf(dbObject.get("_id")));
			if(t1!=null){
				BeanUtils.fillForNotNullObject(t1, object);
				collect.save(covnToDBObject(t1));
			}else{
...
/*	*//**
	...
	@SuppressWarnings("unchecked")
	public final T getByStr(String collectName, String id) {
		if(!init_read){
			initialize_read();
		}
		DBCollection collect = db_read.getCollection(collectName);
		return (T) covnToTObject(collect.findOne(new BasicDBObject("_id", id)));
	}
	/**
	 * 根据某个属性查找第一条记录
	 * 
