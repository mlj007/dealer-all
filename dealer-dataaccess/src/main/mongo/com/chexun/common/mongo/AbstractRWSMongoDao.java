 ...
 * @param <T>
 */
public abstract class AbstractRWSMongoDao<T> {
	protected static Log log = LogFactory.getLog(AbstractMultiMongoDao.class);

	protected String DB_DATA = "website";
	protected Class<T> persistentClass;
	
	@Resource(name = "mongo_write")
	protected Mongo mongo_write;
	@Resource(name = "mongo_read")
	protected Mongo mongo_read;
	...
	protected DB db_write;
	protected DB db_read;
	protected String username;
	protected String password;
	
	public Mongo getMongo_write() {
		return mongo_write;
	}
	...
	protected void initialize() {
		if (!init) {
			ChexunMongoOptions mo_write = (ChexunMongoOptions) mongo_write
					.getMongoOptions();
			username = mo_write.getUsername();
			password = mo_write.getPassword();
			db_write = mongo_write.getDB(mo_write.getDbname());
			db_write.setReadPreference(ReadPreference.SECONDARY) ;
			
			ChexunMongoOptions mo_read = (ChexunMongoOptions) mongo_read
			.getMongoOptions();
			username = mo_write.getUsername();
			password = mo_write.getPassword();
			db_read = mongo_read.getDB(mo_read.getDbname());
			db_read.setReadPreference(ReadPreference.SECONDARY) ;
...
//			System.out.println("db=" + db);
	...
	public final void insert(String collectName, T object) {
		...
		DBCollection collect = db_write.getCollection(collectName);
...
/*	*//**
	...
	public final T get(String collectName, String id) {
		...
		DBCollection collect = db_read.getCollection(collectName);
        ...
        try {
        	if(!values.isEmpty()){
        		...
        		if(object instanceof Integer){
        			...
        			ret = method.invoke(bean, (Object)temp);
        		}
        	}
